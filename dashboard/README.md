...
- **Usage heatmap** — RPC `usage_by_weekday_hour` (reads the `machine_hourly_usage` materialised view, refreshed every 30 min), 28-day window, converted to Europe/London. Each cell = active samples ÷ total samples for that weekday×hour, i.e. "what share of machines were typically printing then". Downtime hours produce no samples and correctly drop out of the denominator.
